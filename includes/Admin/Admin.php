...
namespace UpcomingEventsLists\Admin;
...
class Admin {
	...
	public function initialize_hooks() {
		...
		add_filter( 'manage_edit-' . Event::POST_TYPE . '_columns', array( $this, 'custom_columns_head' ) );
		add_action( 'manage_' . Event::POST_TYPE . '_posts_custom_column',
	...
	public function admin_scripts( $hook ) {
		...
		if ( ( 'post.php' == $hook || 'post-new.php' == $hook ) && ( Event::POST_TYPE == $post->post_type ) ) {
	...
	public function post_type() {
		...
		register_post_type( Event::POST_TYPE, $args );
	}
	...
	public function events_img_box() {
		remove_meta_box( 'postimagediv', Event::POST_TYPE, 'side' );
		add_meta_box( 'postimagediv', __( 'Event Image', 'upcoming-events' ),
			'post_thumbnail_meta_box', Event::POST_TYPE, 'side', 'low' );
	}
	...
	public function add_meta_boxes() {
		add_meta_box( 'sis-event-info-metabox', __( 'Event Info', 'upcoming-events' ),
			array( $this, 'render_event_info_metabox' ), Event::POST_TYPE, 'side', 'core' );
	}
	...
	public function render_event_info_metabox( $post ) {
        ...
        <p>
            <label for="event-start-time"><?php _e( 'Event Start Time:', 'upcoming-events' ); ?></label>
            <input type="time" id="event-start-time" name="event-start-time" step="60"
                   class="widefat" value="<?php echo esc_attr( $event_start_time ); ?>" placeholder="HH:MM">
        </p>
        <p>
            <label for="event-end-time"><?php _e( 'Event End Time:', 'upcoming-events' ); ?></label>
            <input type="time" id="event-end-time" name="event-end-time" step="60"
                   class="widefat" value="<?php echo esc_attr( $event_end_time ); ?>" placeholder="HH:MM">
        </p>
        <p>
	...
	function save_meta_boxes( $post_id ) {
		...
		if ( isset( $_POST['event-start-time'] ) ) {
			update_post_meta( $post_id, 'event-start-time', sanitize_text_field( $_POST['event-start-time'] ) );
		}

		if ( isset( $_POST['event-end-time'] ) ) {
			update_post_meta( $post_id, 'event-end-time', sanitize_text_field( $_POST['event-end-time'] ) );
		}
	}
}
